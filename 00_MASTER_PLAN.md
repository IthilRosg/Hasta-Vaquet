# Master Plan: Hasta-Vaquet VPN Ecosystem

## 1. Vision

Создание независимой, высокопроизводительной и устойчивой к DPI экосистемы VPN —
от уровня кастомного криптографического протокола до полноценного коммерческого продукта.

**Принципы:**
- **Протокол прежде всего.** Защита на уровне wire format — трафик неотличим от шума для любого DPI.
- **Никаких внешних зависимостей.** AES-256-GCM, HMAC, UDP — стандартные примитивы, без сторонних VPN-библиотек.
- **Продукт, а не инструмент.** Конечная цель — пользователь устанавливает клиент и жмёт одну кнопку.

---

## 2. Завершённые фазы

- [x] **Phase 1: Proof of Concept.** Базовый UDP-туннель, передача сырых IP-пакетов, проверка связности.
- [x] **Phase 2: Core Security.** AES-256-GCM, префикс длины для фрагментации, MTU 1300.
- [x] **Phase 3: Global Routing.** 0.0.0.0/0 через Wintun, исключение петли маршрутизации, перехват DNS.
- [x] **Phase 4: Operationalization.** config.json / CLI флаги, Keep-Alive (NAT anti-timeout), агрегированное логирование, корректный shutdown с очисткой маршрутов.
- [x] **Phase 5: DPI Evasion.** HMAC-SHA256 in-band маркер. QUIC fixed bit маска. Асимметричный паддинг (Client 0-40, Server Medium/Chaos). Bloom Filter 64KB 3×FNV-1a. Keep-Alive jitter 10-30с. Результат: 90 Mbps, 0% packet loss.
- [x] **Phase 6: Multi-User Architecture.** Wire Format V6: [HMAC(4)][DynamicID(2)][Nonce(12)][AES-GCM]. DynamicID = ShortID ^ FNV-1a(RoutingSalt+Nonce)[:2]. O(1) маршрутизация map[uint16]*Peer. Per-user ключи. Композитный Bloom (ShortID+Nonce). 1000+ клиентов.
- [x] **Phase 7b: Windows GUI.** Wails + Svelte. Мост app.go. Статистика 2×2 (SPEED/DATA/NETWORK/UPTIME). Профили + dropdown. Native UDP Echo Ping (RTT + loss без ICMP/TCP). IPv6 blackhole. Встроенный wintun.dll. ~90 Mbps.

---

## 3. Активная разработка

- [x] **Phase 7c: Web Management Panel (ЗАВЕРШЕНА).**
  HTTP API встроен в серверный бинарник на отдельном порту. Управление пользователями
  (CRUD над server_config.json с горячей перезагрузкой без рестарта сервера). Статистика
  трафика по каждому peer в реальном времени (ByteIn/ByteOut). Генерация config.json и
  QR-кода для клиентов. Защита: статический admin_token в server_config.json.
  UI: минималистичный HTML/JS встроен через embed.FS, без внешних фреймворков.
  Деплой: https://hasvaq.airydeck.su (Caddy + Let's Encrypt, автообновление сертификата).

**Enhancements:**
- [ ] **Редактирование пользователя.** PUT /api/users/{id} — изменение имени,
  secret_key (ротация), IP, ShortID. Аналог POST, но без удаления сессии.

- [x] **Phase 8: Android Client + Mobile Ecosystem (БАЗОВАЯ ГОТОВНОСТЬ).**
  Рефакторинг core/ под build tags: vpn_interface.go (общая логика), vpn_windows.go
  (текущий Wintun-код), vpn_android.go (VpnService). Gomobile bind с interface-based API.
  Kotlin + VpnService + нативный UI. QR-онбординг через Web-панель.
  Достигнуто: базовое подключение, статистика, kill switch, smart bypass (21 РФ-приложение).

### Phase A: Consolidation (Май 2026) — ЗАВЕРШЕНА
Консолидация кодовой базы и исправление архитектурных проблем:

- [x] **A1. Консолидация криптографии.** Одна реализация Encrypt/Decrypt в `core/crypto.go`.
  `main.go` и `server/server.go` импортируют `core/` вместо своих копий (-206 строк).
- [x] **A2. `math/rand` → `crypto/rand`** для паддинга. Устранение предсказуемости для DPI.
- [x] **A3. Экспорт `Fnv1a16`/`Fnv1a64`** в `core/` для общего доступа с сервером.
- [x] **A4. `core/vpn_other.go`** — заглушки для Linux/неклиентских платформ.
- [x] **A5. Удалён `core/go.mod`** — core стал подпакетом корневого модуля. `go.sum` консолидирован.

### Phase B: Kill Switch + Auto-Reconnect (Май 2026) — ЗАВЕРШЕНА
- [x] **B1. Kill Switch Windows.** При обрыве — удаление default route через route delete.
- [x] **B2. Kill Switch Android.** `VpnService.Builder.setBlocking(true)` + `setAlwaysOn(true)`.
- [x] **B3. Auto-Reconnect.** Экспоненциальный backoff 1s→2s→4s→...→60s cap в `core/vpn.go`.
- [x] **B4. UI-индикация.** Каналы connected/disconnected/reconnecting через StatusListener.

### Phase C: Android — Добивка (Май 2026) — ЗАВЕРШЕНА
- [x] **C1. Smart Bypass MVP.** Хардкод-список из 21 РФ-приложения (Сбер, ВТБ, Тинькофф и др.).
- [x] **C2. `GetStatus()` для Kotlin.** androidListener хранит статус, Kotlin опрашивает.
- [x] **C3. Статистика в UI.** Poll `GetStats()` каждую секунду, отображение 2×2 карточек.

### Phase D: Батчевые фиксы (Май 2026) — ЗАВЕРШЕНА
- [x] **D1. Кумулятивные счётчики трафика.** `CumTx`/`CumRx` — никогда не сбрасываются.
- [x] **D2. `logDrops()`.** Агрегированное логирование отброшенных пакетов (shortID/replay/HMAC/decrypt).
- [x] **D3. Исправление ошибок.** `ListenUDP`, `os.OpenFile` — перестали игнорировать ошибки.
- [x] **D4. Data race.** `speedMu` мьютекс в `handleStats`, `handleReset` сбрасывает CumTx/CumRx.
- [x] **D5. Вложенность функций.** `fmtSpeed`/`fmtUptime`/`resetTraffic` — разделены в index.html.
- [x] **D6. Keep-alive 5-15с.** Уменьшен интервал для мобильных NAT.
- [x] **D7. Ring-buffer loss.** Кольцевой буфер 100 слотов, скользящее окно 30с.
- [x] **D8. Версионирование.** `core/version.go` — v0.2.2, отображается в web/GUI/Android.
- [x] **D9. 16KB page alignment.** AAR с флагом `-Wl,-z,max-page-size=16384`.

---

## 4. Production Hardening — Phase 9

Без этого блока продукт не готов к реальным пользователям:

- [x] **Auto-reconnect.** При обрыве соединения клиент автоматически переподключается
  с экспоненциальным backoff (1s → 2s → 4s → ... → 60s cap). UI показывает «Reconnecting...»
  с таймером следующей попытки.
- [x] **Kill Switch (Windows).** При падении тоннеля — удаление default route.
  Никаких утечек реального IP в момент переподключения.
- [x] **Kill Switch (Android).** `setBlocking(true)` + `setAlwaysOn(true)`.
- [ ] **Windows Installer.** Inno Setup: устанавливает клиент, wintun.dll, создаёт автозапуск,
  ярлык на рабочем столе. Пользователь скачивает один `.exe`.
- [ ] **Auto-update.** Клиент при старте проверяет endpoint на сервере (или GitHub Releases)
  и предлагает/устанавливает обновление. Подписанные релизы.
- [x] **HTTPS для Web-панели.** TLS через Let's Encrypt (Caddy, автообновление сертификата).
  Готово: `https://hasvaq.airydeck.su`.
- [ ] **Срок жизни аккаунтов.** Поле `expires_at` в user-конфиге. Сервер автоматически
  отклоняет пакеты истёкших пользователей без рестарта.
- [ ] **Graceful config reload.** SIGHUP → сервер перечитывает server_config.json и обновляет
  список пиров без прерывания активных соединений.
- [ ] **Systemd unit.** Готовый `hasta-vaquet.service` файл для автозапуска сервера на Linux.

---

## 5. Multi-Server Infrastructure — Phase 10

Один VPS = единая точка отказа:

- [ ] **Несколько серверов в конфиге клиента.** Профиль с массивом нод — клиент видит все
  доступные ему ноды и выбирает сам. При недоступности текущей — failover на следующую.
- [ ] **Выбор ноды по пингу.** Клиент пингует все доступные ноды через Native UDP Echo
  и автоматически подключается к ближайшей.
- [ ] **Node Agent API.** Каждая нода выставляет `/agent/stats` и принимает
  `POST /agent/sync` (пуш списка пиров с панели без рестарта). Защищён отдельным agent_token.
- [ ] **Shared Routing Salt.** Все ноды используют единый RoutingSalt —
  клиент переключается между ними без смены конфига.

---

## 5.5 Node Management & User Access Control — Phase 10b

Эквивалент того, что делают 3x-ui и другие продвинутые VPN-системы. Большая
задача, требует отдельного проектирования перед началом разработки.

- [ ] **Ноды в панели.** CRUD нод через Web UI: добавить/удалить ноду, видеть статус
  (онлайн/оффлайн, кол-во подключённых, нагрузка, версия софта).
- [ ] **Матрица доступа.** Пользователь ⇔ ноды: какой тариф — к каким нодам есть доступ.
  На каждой ноде пользователь имеет отдельный ShortID и ключ (per-node per-user).
- [ ] **Мультинодовый профиль.** Панель генерирует единый JSON-файл/QR
  со всеми доступными пользователю нодами (массив `nodes[]` вместо
  одиночного `server_ip`). Клиент показывает дропдаун со всеми нодами.
- [ ] **Агрегация статистики.** Панель забирает ByteIn/ByteOut с каждой ноды
  и показывает суммарный трафик по пользователю через все ноды.
- [ ] **Тарифная система.** Каждому пользователю можно назначить тариф со
  своим набором нод и лимитами трафика. Основа для монетизации
  из Phase 15.
- [ ] **Обновление без рестарта.** Изменения в матрице доступа мгновенно
  проталкиваются на ноды через agent sync — без перезапуска VPN-процесса.
- [ ] **История соединений / аудит-лог админа.** Кто, когда добавил/удалил
  пользователя, какой трафик по дням. Основа для бизнеса.

---

## 6. Advanced Client Features — Phase 11

### 6.1 Smart Bypass (Anti-Detection Split Tunneling)

Ключевая фича для российских пользователей. Банковские приложения, Госуслуги и другие
РФ-сервисы детектируют VPN и отказывают в работе. Задача — сделать так, чтобы они
вообще не знали о существовании VPN.

**Почему обычный split tunneling (IP-маршруты) не помогает на Android:**
Android OS сама помечает VPN на уровне API — флаг `IS_VPN` / `TRANSPORT_VPN` в
`NetworkCapabilities`. Это читается приложениями через стандартный `ConnectivityManager`
без root. IP-маршруты здесь бессильны — проверка идёт по метаданным интерфейса, не по
адресам. Именно поэтому Happ и другие VPN с IP-based split tunneling всё равно
палятся банковскими приложениями.

**Правильное решение для Android:** `VpnService.Builder.addDisallowedApplication(pkg)`.
Android создаёт для исключённого приложения отдельный сетевой контекст, полностью
минуя VPN-интерфейс. Сбер вызывает `hasTransport(TRANSPORT_VPN)` для своей сети —
получает `false`. Это изоляция на уровне ядра, не routing trick.

**Для Windows/Linux:** IP/CIDR-маршруты через реальный шлюз работают, потому что
десктопные РФ-приложения в основном проверяют серверную сторону (GeoIP), а не
клиентские API.

⚠️ **Из vpn_detection.md (раздел 7.6):** Split tunneling создаёт дополнительные
аномалии в таблицах маршрутизации (маршруты к нестандартным шлюзам для разных
подсетей). Это косвенный признак VPN. Однако для РФ-пользователей это необходимый
компромисс: наша реальная защита от DPI — на уровне wire format, а routing-anomaly
является лишь косвенным признаком, недостаточным для блокировки.

- [ ] **Режим Auto.** При первом запуске (и после обновления списка) сканировать
  установленные приложения (Android: `PackageManager.getInstalledPackages()`) и
  автоматически применять bypass для найденных РФ-приложений из кюрированного
  JSON-списка. На Windows/Linux — автоматически скачать и применить IP/CIDR/domain
  список без участия пользователя. Уведомить: «Найдено N приложений, bypass применён».

- [ ] **Режим Advanced.** Открывается в настройках. Android: список всех установленных
  приложений с тогглами (авто-найденные отмечены, пользователь добавляет/убирает
  любые). Windows/Linux: таблица с авто-списком + поле добавления своих IP/CIDR/доменов.
  Пользователь видит что именно в bypass и полностью это контролирует.

- [ ] **Bypass List — единый JSON-формат** для всех платформ:
  ```
  {
    "version": "YYYY.MM.DD",
    "android_packages": [{"package": "ru.sberbank.spasibo", "name": "Сбербанк", "category": "banking"}, ...],
    "ip_ranges":         [{"cidr": "5.8.0.0/21", "name": "Сбербанк", "asn": 43478}, ...],
    "domains":           ["sberbank.ru", "gosuslugi.ru", ...],
    "dns_overrides":     [{"domain": "broken.ru", "ip": "1.2.3.4", "comment": "..."}]
  }
  ```
  Сами списки наполняются отдельно по мере тестирования.

- [ ] **Auto-update списка.** Клиент проверяет версию при запуске и раз в 24 часа.
  Windows/Linux: новые маршруты применяются горячо, без reconnect.
  Android: изменение `addDisallowedApplication` требует пересоздания VPN-сессии —
  показываем уведомление «Список обновлён, применится при следующем подключении».

- [ ] **HMAC-подпись списка.** Список подписывается ключом, зашитым в бинарник клиента.
  Клиент отклоняет список с невалидной подписью. Защита от MITM и компрометации сервера.

- [ ] **Split DNS.** РФ-домены → `77.88.8.8` (Яндекс DNS) напрямую — возвращает
  российские CDN IP, приложения видят «правильные» адреса. Всё остальное →
  `1.1.1.1` через VPN.

- [ ] **Hosts / DNS Override.** Для сервисов, недоступных даже через VPN (IP сервера
  заблокирован у сервиса): домен добавляется в bypass — трафик идёт напрямую с
  реальным российским IP. Для сервисов с DNS-отравлением: статический override
  в hosts-файле → известный рабочий IP. Список проблемных сервисов наполняется
  отдельно.

- [ ] **Два подрежима Exclude/Include:**
  - **Exclude (дефолт для РФ):** весь трафик через VPN, кроме bypass-списка.
    Российские сервисы видят реальный IP, заблокированные сайты идут через VPN.
  - **Include:** только bypass-список через VPN, остальное напрямую.
    Для тех, кому нужен доступ к конкретным заблокированным ресурсам без
    полного туннелирования.

### 6.2 User Portal — панель для клиентов (до Phase 15)

Отдельная страница на домене (например `/portal/`) для конечных пользователей.
Не путать с Admin Panel — пользователь видит только свои данные.

- [ ] **Личный кабинет.** Пользователь вводит свой `config_token` (формируется из
  `sha256(secret_key)[:8]` или отдельный UUID, генерируется в конфиге). Просматривает
  статус подписки, оставшийся трафик, дату истечения.
- [ ] **Скачать config.json.** Кнопка скачивания своего конфига.
- [ ] **QR-код.** Отображение QR с конфигом для мобильного клиента.
- [ ] **Статистика.** Трафик за текущий период, uptime соединения (если онлайн),
  количество использованных нод.
- [ ] **Смена ключа.** Возможность запросить ротацию `secret_key` — старый ключ
  деактивируется, новый применяется через agent sync.
- [ ] **Автовход через ссылку.** `https://domain/portal/?token=...` — редирект и автологин.
  Удобно для QR-кодов с длинными URL.

---

### 6.3 Остальные фичи Phase 11

- [ ] **Autoconnect при запуске системы.** Запись в реестр (Windows) / systemd user
  unit (Linux) / автозапуск (Android).
- [ ] **Autoconnect на недоверенных сетях.** Клиент определяет тип сети (незнакомый
  SSID, публичный Wi-Fi) и автоматически включает VPN.
- [ ] **Bandwidth monitor в UI.** Трафик за сессию и за месяц. Уведомление при
  превышении лимита.
- [ ] **История соединений.** Лог последних N сессий: время, сервер, трафик, RTT.

---

## 7. Advanced DPI Evasion — Phase 12

Исследовательская фаза. Цель — сделать трафик неотличимым даже при ML-анализе:

- [ ] **Traffic shaping.** Нормализация размеров пакетов к фиксированным значениям (256/512/1024).
  Устраняет статистические признаки реального трафика в размерах пакетов.
- [ ] **Timing obfuscation.** Искусственные задержки между пакетами по случайному распределению.
  Разрушает timing-fingerprint соединения.
- [ ] **Constant bitrate padding.** В периоды низкой активности — генерация dummy-пакетов
  до фиксированного битрейта. Сравнивает трафик с видеостримингом.
- [ ] **Domain Fronting / CDN routing.** Направление UDP через CDN-узел (Cloudflare/Fastly).
  Реальный IP сервера скрыт даже от ISP.
- [ ] **Protocol mimicry.** Первые N байт каждого соединения — валидный TLS ClientHello
  (с нашими данными внутри). Пассивные DPI-системы классифицируют как HTTPS.

### Phase 12a: Adversarial DPI Probing (запомнить, вернуться)

**Идея:** Метод чёрного ящика для выявления решающих правил DPI (ТСПУ).
Ставится сервер снаружи РФ, клиент внутри РФ. Протокол модифицируется,
замеряется — дошёл пакет или нет. Каждый бит фидбека = 1 бит информации о правиле DPI.

**Стратегия:**
1. Зафиксировать все параметры протокола, менять по одному (порт, первые байты, размер, интервал)
2. Построить карту границ решений DPI (при каких значениях пакет блокируется)
3. Genetic Algorithm: поколения конфигов → отбор лучших → скрещивание → мутация
4. На выходе — конфиг, неотличимый от HTTPS для данного DPI

**Параметры для эволюции:**
- Первые 16 байт пакета (DPI смотрит начало)
- Распределение размеров пакетов
- Интервалы между пакетами (distribution)
- TLS v1.3 session ticket как обёртка
- Наличие TCP-рукопожатия перед UDP

**CPU-затраты:** 0% на продакшене, только в лаборатории.

---

## 8. Platform Expansion — Phase 13

- [ ] **macOS клиент.** utun API (аналог Wintun для macOS). Network Extension framework.
  Wails + Svelte (переиспользовать GUI из Windows). Ограничение: требует подписанного
  приложения и notarization от Apple.
- [ ] **Linux GUI.** GTK или тот же Wails. Использовать /dev/net/tun. Целевая аудитория:
  технические пользователи, которые сейчас используют CLI.
- [ ] **Browser Extension.** SOCKS5-прокси внутри расширения. Не требует прав администратора.
  Ограниченная защита (только браузерный трафик), но нулевой порог установки.

---

## 9. iOS — Phase 14 (сложная фаза)

- [ ] **Network Extension.** Единственный легальный способ VPN на iOS — через
  `NEPacketTunnelProvider`. Требует entitlements от Apple (платная программа разработчика).
- [ ] **Общее Go-ядро.** Та же gomobile-библиотека из Phase 8, но скомпилированная для iOS.
  Crypto и wire format идентичны — новый код только для платформы.
- [ ] **App Store.** Публикация требует соответствия правилам Apple, включая политику
  конфиденциальности, отсутствие обхода ограничений контента (тонкий момент для VPN).

---

## 10. Commercial Layer — Phase 15

- [ ] **User management с подпиской.** `expires_at`, `quota_bytes` в конфиге пользователя.
  Сервер автоматически блокирует истёкших / исчерпавших квоту без рестарта.
- [ ] **Платёжный шлюз.** Интеграция Stripe (карты) + криптовалюта (USDT/BTC).
  Автоматическая генерация конфига и QR после оплаты.
- [ ] **Реферальная система.** Пользователь получает расширение подписки за каждого
  приведённого друга. Отслеживается через уникальный ref_code в конфиге.
- [ ] **Reseller панель.** Отдельный уровень доступа в Web-панели: партнёр создаёт
  пользователей в своей квоте, видит только свою статистику.
- [ ] **Уведомления.** Email/Telegram-бот: уведомление за N дней до истечения подписки,
  при исчерпании квоты, при подозрительной активности.

---

## 11. Current Task

**Текущее состояние (Май 2026):**
- [x] Phase 8: Android базовый клиент — ✅
- [x] Phase A: Consolidation — ✅
- [x] Phase B: Kill Switch + Auto-Reconnect — ✅
- [x] Phase C: Добивка Android — ✅
- [x] Phase D: Батчевые фиксы — ✅
- [ ] Phase E: Production Hardening (installer, rate limiting, quotas, systemd) — ФОКУС
- [ ] Phase D: Полевое тестирование против DPI РКН — НЕОБХОДИМО
- [ ] Phase F: Коммерциализация — отложено

**Ближайшие задачи (приоритет):**
1. Rate limiting на сервере (защита от UDP flood)
2. `expires_at` + `quota_bytes` в конфиге пользователя
3. Windows Installer (Inno Setup)
4. Systemd unit для сервера
5. Полевое тестирование с 5-10 реальными пользователями в РФ

**Версия:** `v0.2.3` — определена в `core/version.go`, отображается во всех клиентах.
